...
class AppService {
  ...
  Future<void> processChooseMultiImage({BuildContext context}) async {
    ...
    await ImagePicker().pickMultiImage().then((value) async {
      print('##4aug จำนวนภาพที่เลือก ---> ${value.length}');
      ...
      for (var element in value) {
        ...
        if (nameImage.contains('_')) {
          bool response = await checkNameImage(
              nameImage: nameImage); //true มีชื่อนี่อยู่แล้ว

          if (!response) {
            var strings = nameImage.split('_');
            if (strings.isNotEmpty) {
              String fourDigi = strings[0].substring(0, 4);
      ...
      if (appController.xFiles.isNotEmpty) {
        MyDialog(context: context).normalDialot(
          title: 'title',
          subTitle: 'จำนวนภาพที่เลือก ---> ${appController.xFiles.length}',
          ...
          firstButton: WidgetTextButton(
            ...
            pressFunc: () async {
              ...
              for (var i = 0; i < appController.xFiles.length; i++) {
                String urlAPIsave =
                    'https://www.androidthai.in.th/fluttertraining/sinno/saveImageJob.php';
                ...
                print('##4aug responseUpLoad ---> $responseUpload');
                ...
                await dio.Dio().get(urlApiInsert).then((value) {
                  Get.snackbar('Upload Success', 'message');
                });
              } // for
            },
          ),
        ...
        Get.snackbar('No Image', 'No Image');
      }
    });
  }
  ...
  Future<bool> checkNameImage({String nameImage}) async {
    ...
    bool response = false; // false ---> ชื่อใช้ได้ไม่ซ้ำ
